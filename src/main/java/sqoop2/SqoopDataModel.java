...
//        linkConfig.getStringInput("linkConfig.connectionString").setValue("jdbc:mysql://60.205.184.214:3306/redis");
        linkConfig.getStringInput("linkConfig.connectionString").setValue("jdbc:mysql://60.205.184.214:3306/redis");
...
//保存连接
    ...
    public static String createMysqlToHdfsJob(MLink fromLink, MLink toLink) {
        ...
        job.setName("zhaojixun-job" + UUID.randomUUID());
        job.setCreationUser("zhaojixun");
        //获取数据源配置对象fromJobConfig，并配置数据库名和表名，以及字段名
        MFromConfig fromJobConfig = job.getFromJobConfig();
        fromJobConfig.getStringInput("fromJobConfig.schemaName").setValue("redis");
        fromJobConfig.getStringInput("fromJobConfig.tableName").setValue("user");
        fromJobConfig.getListInput("fromJobConfig.columnList").setValue(Arrays.asList("username", "password"));
    ...
    public static String createHdfsToMysqlJob(MLink fromLink, MLink toLink) {
        MJob job = client.createJob(fromLink.getName(), toLink.getName());
        job.setName("zhaojixun" + UUID.randomUUID());
        job.setCreationUser("zhaojixun");
        MFromConfig fromJobConfig = job.getFromJobConfig();
        fromJobConfig.getStringInput("fromJobConfig.inputDirectory").setValue("/sqoopDir");
        MToConfig toJobConfig = job.getToJobConfig();
        toJobConfig.getStringInput("toJobConfig.tableName").setValue("user1");
